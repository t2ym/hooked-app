...
class TargetConfig extends Injectable(Traceable(Configurable(GulpDefaultRegistry, 'thin-hook'))) {
  ...
  _configure() {
    ...
    this.assign('path')({
      ...
      frontend: 'frontend',
      keys: 'keys',
      compatLib: 'compat-lib', // custom compatible libraries are built and stored here
    ...
    .assign('url')({
      [TargetConfig.needResolution]: true,
      root: '/', // usually root path should be in an upper directory like '/'
      components: '/node_modules',
      mappings: () => {
        let mappings = [
    ...
    .assign('validationService')({
      ...
      port: 8082,
      /*
        entryURL: for commands.https
          https://${SERVER_HOST}:${server.port}${url.root} - no proxy
        proxiedEntryURL: for commands.http
          https://${SERVER_HOST}${url.root} - proxy at port 443
       */
      entryURL: `https://${this.server.host}${this.server.port != 443 ? ':' + this.server.port : ''}${this.url.root + (this.url.root.endsWith('/') ? '' : '/')}`,
      proxiedEntryURL: `https://${this.server.host}${this.url.root + (this.url.root.endsWith('/') ? '' : '/')}`, // assuming port 443
      entryURLProperty: [ 'entryURL', 'proxiedEntryURL' ][1], // 0 for commands.https, 1 for commands.http
      buildURL: `https://localhost${this.server.port != 443 ? ':' + this.server.port : ''}${this.url.root + (this.url.root.endsWith('/') ? '' : '/')}`,
    })
    ...
    .assign('no-hook-authorization')({
      hash: {
        //"https://cdnjs.cloudflare.com/ajax/libs/vis/4.18.1/vis.min.js": "db82c32a68bc3ddbb2232a947c3cdeb88f5ee7ba41d05d686146507687a6897f", // CORS no-hook scripts are highly discouraged
        "location = \"about:blank\";": "4f0395d52a8c1c7edaacacade9c31fe18555b79ce963dfb1abaaa34990993374", // TODO: deprecate and generate from "about-blank-redirector.js"
      },
      hints: [ // optional hints to sort the entries; usually unnecessary
      ],
      passcode: 'ab02c107ea633ed697acc12e1b3de1bcf2f0ef7cafe4f048e29553c224656ecd7a',
      sourceMap: `[\n` +
    ...
    .assign('bootstrap-js')({
      ...
      hangUpOnEmbedAndObjectElement: true,
      /*
      ...
      */
      omitSuperfluousClosingHtmlTags: true,
      importMapsJson: true,
      additionalParameters: true, // load additional parameters from config/bootstrap-js/parameters.js
    ...
    .assign('url-parameters')({
      [TargetConfig.needResolution]: true,
      inline: false, // comment out this line to disable inline script injection
      attributes: {
        src: (plugin, targetPlugin, targetConfig) => this.getPluginUrl(plugin, targetPlugin),
        integrity: (plugin, targetPlugin, targetConfig) => this.getIntegrity(plugin),
      },
    ...
    .assign('bundles')({
      targets: [
        ...
        [ 'src/main.js',        'app-bundle.js',          'rollup'    ],
      ...
      'enhanced-resolve': {
        ...
        context: path.resolve(this.path.base, this.path.root),
      },
      browserify: {
    ...
    .assign('hook-worker-js')({
      type: 'hookWorker',
      //customHookWorkerHandler: true, // edit config/hook-worker-js/hookWorkerHandler.js to customize
      searchParams: {
        'no-hook': true,
      },
      targetTypes: {
    ...
    */
    ...
    .assign('inject')({
      ...
      bootstrap: [
        /* hooked scripts in SVG in iframe/object/embed will not run but inline SVG images in HTML remain clean
        'hook-min-js',
        'no-hook-authorization',
        'context-generator-js',
        'bootstrap-js',
    ...
    .assign('commands')({ // these commands are registered as gulp.task('command', shell.task(this.commands[command])) in inject plugin initialization unless already registered with the name
      ...
      "buildServer": `${this.server.port < 1024 ? this.server.privilegedPortBinder + ' ' : ''}node ${path.resolve(this.path.base, this.path.backend, this.server.serverJs)} -p ${this.server.port} \\
        -m build -P https -H localhost${this.server.port == 443 ? '' : ':' + this.server.port}`,
      "httpServer": `${this.server.port < 1024 ? this.server.privilegedPortBinder + ' ' : ''}node ${path.resolve(this.path.base, this.path.backend, this.server.serverJs)} -p ${this.server.port} \\
        -m server -c ${this.server.concurrency} -H ${process.env['SERVER_HOST'] || 'localhost'} \\
        --clientHints 'Sec-CH-UA-Full-Version-List' `,
      "httpsServer": `${this.server.port < 1024 ? this.server.privilegedPortBinder + ' ' : ''}node ${path.resolve(this.path.base, this.path.backend, this.server.serverJs)} -p ${this.server.port} \\
        -m server -c ${this.server.concurrency} -P https -H ${process.env['SERVER_HOST'] || 'localhost'}${this.server.port == 443 ? '' : ':' + this.server.port} \\
        --clientHints 'Sec-CH-UA-Full-Version-List' `,
      "monitorServer": `${this.server.port < 1024 ? this.server.privilegedPortBinder + ' ' : ''}node ${path.resolve(this.path.base, this.path.backend, this.server.serverJs)} -p ${this.server.port} \\
        -m monitor -c ${this.server.concurrency} -P https -H wildcard.${process.env['SERVER_HOST'] || 'localhost'}:${this.server.port}`,
      "debugServer": `${this.server.port < 1024 ? this.server.privilegedPortBinder + ' ' : ''}node --inspect-brk=${this.server.devToolsHostPort} ${path.resolve(this.path.base, this.path.backend, this.server.serverJs)} -p ${this.server.port} \\
      ...
      "cacheBundleGeneration": `node ${path.resolve(this.path.base, this.path.backend, 'cacheBundleGeneration.js')} ${this.validationService.buildURL}`,
      "loadOnly": `node ${path.resolve(this.path.base, this.path.backend, 'cacheBundleGeneration.js')} loadOnly ${this.validationService.buildURL}`,
